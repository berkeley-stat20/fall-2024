...
The sample standard deviation $s$ is closely related to the sample variance $s^2$: we've just taken the square root. One reason for using the sample standard deviation is that it is more interpretable than the sample variance since it is measured in the original units rather than units squared.
...
If $x_i - \bar{x}$ is large, then $(x_i - \bar{x})^2$ will be *very* large. Therefore, $s^2$ and $s$ can lead an inflated sense of the spread of *most* of the data. Here are two measures of spread which counteract this. 
